...
- Added sections:
  - Core Principles: VI. Autonomous Clarification (Claude Decides, Not
    Blocks)
...
- Templates requiring updates:
  ...
  - ✅ .specify/templates/spec-template.md (NEEDS CLARIFICATION marker mechanism
    unchanged — this principle governs how markers get resolved during
    execution, not the template's structure)
  - ✅ .specify/templates/tasks-template.md (no constitution-specific references)
  - ⚠ .claude/skills/speckit-*/SKILL.md (several, e.g. speckit-specify,
    speckit-clarify, contain literal "wait for user response" instructions
    for NEEDS CLARIFICATION markers; these files are spec-kit-managed and
    regenerated by `specify upgrade`, so they are not hand-edited — per
    Governance below, this constitution supersedes that instruction at
    execution time: Claude resolves clarifications itself instead of
    waiting, except where Principle VI's own exceptions apply)
- Follow-up TODOs:
...
### VI. Autonomous Clarification (Claude Decides, Not Blocks)

When any spec-kit workflow (specify, clarify, plan, tasks, implement,
checklist, analyze, or any other) raises a clarifying or open question,
Claude MUST resolve it itself — using this constitution, the spec,
`CONTEST.md`/competition rules, prior recorded decisions, and standard
practice for competitive ML/game-agent work — rather than pausing the
workflow to ask the user. Every autonomous resolution MUST be recorded
where the question arose (e.g., replacing a `[NEEDS CLARIFICATION: ...]`
marker with the chosen answer plus a one-line rationale, or an Assumptions
entry) so the decision stays visible and reversible.

Claude MUST still stop and ask the user, or wait for explicit
authorization, only when:

- The action requires credentials, account access, or is irreversible/
  destructive (e.g., submitting to Kaggle, force-pushing, deleting data,
  spending submission budget) and only the user can authorize it.
- The user has explicitly asked to be consulted on that specific decision,
  or has overridden this principle for the current task.
- No reasonable default exists and every option would materially affect
  legal/rules compliance (Principle IV) — in this case Claude MUST still
  proceed using the most conservative, rules-compliant interpretation,
  recording the assumption, rather than blocking outright.

**Rationale**: Pausing for every ambiguous decision stalls iteration speed,
which directly works against Principle I (Leaderboard-Driven Iteration) and
Principle V (Time-Boxed Simplicity) under a hard competition deadline.
Autonomous, recorded, reversible defaults preserve momentum without
...
non-negotiable regardless of time pressure. Principle VI's exceptions
(credentialed/irreversible actions, explicit user override) are likewise
non-negotiable — autonomy under Principle VI never extends to bypassing
them.
