...
def _hire_target(obs: GameObservation) -> int:
    ...
    return min(
        MAX_HANDS,
        _count_placed_animals(obs) + _pending_animals(obs) + HAND_SLACK + PLOT_WORKERS,
    )
...
def plot_tiles(board_size: int) -> tuple[tuple[int, int], ...]:
    ...
    selected = candidates[:PLOT_SIZE]
    # Re-sort by angle around the selection's own centroid, so slicing
    # this sequence into per-worker blocks (worker_block, below) keeps
    # each worker's tiles adjacent on the board rather than scattered
    # across opposite sides of the ring.
    cx = sum(t[0] for t in selected) / len(selected)
    cy = sum(t[1] for t in selected) / len(selected)
    selected.sort(key=lambda t: math.atan2(t[1] - cy, t[0] - cx))
...
def _plot_tile_work(obs: GameObservation, pos: tuple[int, int]) -> str | None:
    """What the plot tile at `pos` needs now, or None."""
    tile = obs.tile_at(*pos)
    if tile is None:
        return "PLANT" if obs.seeds.get(PLOT_CROP, 0) > 0 else None
    kind = tile_kind(tile)
    if kind == "WEED":
        return "DIG"
    if kind != "PLANT" or not isinstance(tile, dict) or tile.get("crop") != PLOT_CROP:
        return None
    cd = CROPS[PLOT_CROP]
...
def _decide_crop_action(obs: GameObservation, pos: tuple[int, int], block: tuple) -> list[str]:
    """A crop worker tends its own block and nothing else."""
    if not block:
        return ["PASS"]
    if pos in block:
        work = _plot_tile_work(obs, pos)
        if work == "DIG":
            return ["DIG"]
        if work == "PLANT":
            return ["PLANT", PLOT_CROP]
        if work == "WATER":
            return ["WATER"]
        if work == "HARVEST":
            return ["HARVEST"]
    needy = [t for t in block if _plot_tile_work(obs, t) is not None]
    if needy:
...
def _decide_hand_action(
    ...
    for animal, info in ANIMALS.items():
        ...
        if target:
            claimed.add(target)
            return _step_toward(pos, target)
        if kind == "EMPTY" and pos not in plot_tiles(len(obs.my_farm["tiles"])):
...
def baseline_strategy(obs: GameObservation) -> dict:
    ...
    claimed.update(plot)  # herd duty must never build a pasture on the crop plot
    ...
    duty = _placement_duty_index(obs, exclude=crop_workers)
    ...
    for idx in order:
        if idx in crop_workers:
            actions[idx] = _decide_crop_action(
                obs, positions[idx], worker_block(plot, crop_rank[idx], len(crop_workers))
            )
        else:
