...
# was introduced. Under v7's much smaller economy, 1.0 (no real buffer)
# reproduced the original cash-crash failure mode (mean ~2.4k); 2.0 was
# the best value found there.
#
# v17: re-swept against a v16 base that ALREADY races land acquisition
# faster (see LAND_RESERVE_MULT) -- and the verdict inverted. Replay
# evidence (both top agents studied) shows real opponents running with
# near-zero cash for the first ~10 days as a matter of course, racing
# land, hands and animals simultaneously rather than keeping a reserve.
# 1.0 -> 90% against the land-rush base (40 seasons), confirmed 81%
# at 100 (pooled 117W/23L, 83.6% over 140). The old cash-crash failure
# mode does not reproduce here: mean money is HIGHER at 1.0 than at
# 2.0, not lower -- traced directly, money is near zero for days 0-8
# then grows smoothly once the herd and land investment completes,
# matching the real replays' shape almost exactly. The mechanism this
# guarded against evidently needs the OLD, slower land-acquisition
# pace to bite; with land racing ahead of it, the "reserve" was pure
# opportunity cost, not safety margin.
REINVEST_RESERVE = 1.0
MAX_STRUCTURES = 15
...
# 20-season batches.
MAX_QUADRANTS = 3
# v17: replay evidence (both top agents studied) shows land bought with
# money near zero for the first ~10 days -- a much thinner safety
# margin than the 2x this used to require. Swept 1.0/1.1/1.2/1.5/2.0
# against v16: 25%/30%/62%/55%/50% (20-40 seasons), then confirmed 1.2
# at 100 seasons (55W/35L/10T) -- pooled with the 40-season screen,
# 80W/43L/17T, 57.1% over 140 seasons. Modest but real: racing land
# acquisition faster (not buying MORE land -- MAX_QUADRANTS is
# unchanged) helps a little.
LAND_RESERVE_MULT = 1.2
...
# v15: unstick the herd. These three changes only work TOGETHER, which
# is why each measured as a loss on its own across two sessions.
#
# A bought animal only leaves the shed if a unit happens to stand on a
# shed tile while NO animal is unfed -- and past a certain herd size
# someone is always unfed at dawn, so an animal sat unplaced for 78% of
# v13's turns. Since _market_orders refuses to buy while one is pending,
# the herd froze at 12-13 while the bank climbed past 46,000.
#
# Unsticking it ALONE measured 0W/20L, three separate times, because the
# resulting bigger herd could not be fed: the shed was serialised to one
# unit per turn (v14 fixed that), and the extra hands meant to feed it
# never materialised, because the hour-0 hire queue is truncated by
# maxMarketOrdersPerTurn -- v13 ran 7-8 hands however high the target
# was. Adding hands ALONE measured harmful too, for the same reason in
# reverse: with the herd frozen and the shed serialised, extra hands had
# nowhere to go and idled.
#
# All three together: herd 8.8 -> 11.0, pending animals 0.80 -> 0.23,
# idle unit-turns 34.2% -> 15.8%.
PLACEMENT_DUTY = True
# Hires are spread over the first turns of the day to get past the
# per-turn order cap, and emitted AFTER the sell orders -- filling the
# cap early in the day blocks BUY_PRODUCT WHEAT and the herd starves
# (15% vs 65% for otherwise identical agents).
HIRE_WINDOW_HOURS = 3
# The n-th hire of a day costs fib(n), and hands are re-hired every
# morning, so the last few are charged daily. On v13 more than ~7 hands
# was harmful; on top of v14's shed fix the useful ceiling is higher.
HIRE_COST_CEILING = 55
...
# v16, attempt 14: grow WHEAT itself, on a small role-persistent plot,
# rather than a market crop.
#
# Thirteen earlier attempts to add a SECOND MARKET (MELON, STRAWBERRY)
# all failed: any allocation splitting workers between crops and animals
# either starves the herd (dedicated workers) or fails to maintain the
# crop (idle-time workers, or role-persistent workers competing for the
# same early hire-capital the herd needs to compound).
#
# WHEAT is a different kind of crop economically. It is not a second
# market -- it directly offsets an ALREADY-MEASURED expense: v15 buys
# ~330 units/season for ~12,960 (about half of all outflow) at a price
# that climbs 25 -> 57 because both farms and five of the eight shop
# types drain the same market. And it is a different kind of crop
# LABOUR-wise: seed 10, first_yield_day 2, max_yield_day 4 -- a planting
# ties up a worker for ~5 days total (1 plant + up to 4 waters), against
# MELON's ~13 or STRAWBERRY's indefinite hold. A worker that finishes a
# WHEAT cycle is free again in under a week, so the capital and labour
# competing with the herd's day-0-15 compounding phase is much smaller.
PLOT_CROP = "WHEAT"
PLOT_WORKERS = 3
PLOT_SIZE = 12
# Tiles nearest the shed stay free for pastures -- every animal is fed
# from the shed daily, and fencing the near ring measured 0W/20L.
PLOT_MIN_SHED_DIST = 2
# WHEAT seed is cheap (10/unit) so a full 8-seed batch costs at most 80
# regardless of plot size -- a much smaller claim on early capital than
# MELON (640) or STRAWBERRY (800), so no separate capital-floor gate is
# needed here.
PLOT_SEED_BATCH = 8

# v18 probe: an optional SECOND plot for a second crop, laid out on the
# next-nearest ring of candidate tiles after the wheat plot, with its own
# dedicated workers (hired on top, not taken from the herd) and a cash
# floor on its seed purchases so it cannot starve the early herd build.
# PLOT2_SIZE = 0 disables it entirely (exact v17 behaviour).
#
# Motivation is real-field, not self-play: v17 went 1W/7L in real
# matches whose towns barely buy MILK or WOOL (<= 3 shop-units between
# them) -- it has nothing else to sell, and milk/wool crash to 1 by day
# ~18 while every animal still eats a ~50 wheat/day ration. The
# opponents that beat it in those towns sold STRAWBERRY and MELON.
PLOT2_CROP = "STRAWBERRY"
PLOT2_SIZE = 0
PLOT2_WORKERS = 0
PLOT2_CAPITAL_FLOOR = 1500
PLOT2_SEED_BATCH = 6
...
# v18: feed an animal only when the feed pays for itself. The engine
# (_daily_refresh_animals) produces an animal's BASE unit on schedule
# whether or not it was fed; feeding only (a) resets the escape counter --
# two consecutive unfed days and it escapes -- and (b) together with CARE,
# banks a +1 bonus unit that is paid out on the next FED production day.
# Fertilizer appears daily regardless. So:
#   - a production night must be fed (it cashes in the banked bonus),
#   - a day after an unfed day must be fed (or the animal escapes),
#   - any other day's feed buys exactly one extra product unit, and is
#     only worth a wheat when that product trades at >= FEED_VALUE_RATIO
#     times the wheat price.
# Self-play and real matches alike crash MILK/WOOL to single digits by
# day ~15-18 while WHEAT trades ~50, so v17 was feeding every animal
# every day at a loss for the second half of the season.
FEED_VALUE_RATIO = 1.5
BUY_CUTOFF_MARGIN = 2


def _production_tonight(tile: dict, day: int) -> bool:
    a = ANIMALS[tile["animal"]]
    since = day + 1 - tile.get("placed_day", 0) - a["first_yield_day"]
    return since >= 0 and since % a["interval"] == 0


def _should_feed(obs: GameObservation, tile) -> bool:
    if not (isinstance(tile, dict) and tile.get("animal")) or tile.get("fed_today"):
        return False
    if FEED_VALUE_RATIO <= 0:
        return True
    # Final day: nothing produced tonight can ever be sold, and an escape
    # after the season ends costs nothing.
    if obs.day >= SEASON_DAYS - 1:
        return False
    if tile.get("consecutive_unfed", 0) >= 1 or _production_tonight(tile, obs.day):
        return True
    product = ANIMALS[tile["animal"]]["product"]
    prices = obs.market_prices
    wheat = prices.get("WHEAT", MARKET_PARAMS["WHEAT"]["base"])
    return prices.get(product, MARKET_PARAMS[product]["base"]) >= FEED_VALUE_RATIO * wheat


def _count_unfed_animals(obs: GameObservation) -> int:
    """Animals that still need (and are worth) a feed today."""
    return sum(1 for row in obs.my_farm["tiles"] for tile in row if _should_feed(obs, tile))
...
def _hire_target(obs: GameObservation) -> int:
    ...
    return min(
        MAX_HANDS,
        _count_placed_animals(obs) + _pending_animals(obs) + HAND_SLACK + PLOT_WORKERS + PLOT2_WORKERS,
    )
...
def _fib(n: int) -> int:
    """Matches the engine's hire-cost schedule: 1, 1, 2, 3, 5, 8, ..."""
    a, b = 1, 1
    for _ in range(n):
        a, b = b, a + b
    return a
...
def _hire_orders(obs: GameObservation, money: float) -> list[list]:
    """HIRE orders for this turn, cost-bounded against the fib schedule.

    Hands are re-hired every morning (the engine clears farm["hands"]
    nightly) and the hour-0 queue is truncated by maxMarketOrdersPerTurn,
    so v13 runs 7-8 hands whatever the target says. Emitted AFTER the
    sells: filling the order cap early in the day blocks BUY_PRODUCT
    WHEAT and the herd starves (15% vs 65%, otherwise identical agents).
    """
    if obs.hour > HIRE_WINDOW_HOURS or money < HIRE_MONEY_RESERVE:
        return []
    hires_today = obs.my_farm.get("hires_today", 0)
    orders: list[list] = []
    # No affordability check here: the engine's _do_hire already refuses a
    # hire the farm cannot pay for, and duplicating that logic made this
    # queue fewer hires than v13/v14 did in the cash-poor early game.
    for n in range(hires_today, _hire_target(obs)):
        if _fib(n) > HIRE_COST_CEILING:
            break
        orders.append(["HIRE"])
    return orders


def _placement_duty_index(obs: GameObservation, exclude: set = frozenset()) -> int | None:
    """Which single HERD unit (0 = farmer, 1+ = hands) fetches a bought
    animal.

    Feeding must outrank fetching or the herd starves, so exactly ONE
    unit is excused from feed duty -- the one already nearest the shed --
    and only while an animal is actually waiting. `exclude` keeps crop
    workers out of the running: they ignore this flag entirely (see
    _decide_crop_action), so picking one silently wastes the assignment
    and no herd unit ever collects the animal -- a real bug from an
    earlier attempt.
    """
    if not PLACEMENT_DUTY or _pending_animals(obs) <= 0:
        return None
    inventories = [obs.farmer_inventory()] + [
        obs.hand_inventory(i) for i in range(len(obs.hand_positions()))
    ]
    if any(any(inv.get(a, 0) > 0 for a in ANIMALS) for inv in inventories):
        return None  # already in progress
    sheds = shed_tiles(len(obs.my_farm["tiles"]))
    positions = [obs.farmer_position()] + obs.hand_positions()
    candidates = [i for i in range(len(positions)) if i not in exclude]
    if not candidates:
        return None
    return min(
        candidates,
        key=lambda i: min(
            abs(positions[i][0] - t[0]) + abs(positions[i][1] - t[1]) for t in sheds
        ),
    )


def plot_tiles(board_size: int) -> tuple[tuple[int, int], ...]:
    """The wheat plot: compact, on land we actually buy, just outside the
    ring reserved for pastures.

    Three constraints, each learned from a bug in an earlier attempt that
    produced zero crops. The plot must be on OWNABLE land -- anchoring on
    the tile furthest from the shed picks the SE quadrant, which
    MAX_QUADRANTS=3 never unlocks. It must be COMPACT and in a spatially
    CONTIGUOUS order -- taking the furthest tiles picks the four corners
    of the board, and sorting only by shed-distance produces a sequence
    that slices into scattered per-worker blocks. And it must be near
    enough that a worker's daily walk from the shed is short.
    """
    sheds = shed_tiles(board_size)
    half = board_size // 2

    def shed_dist(t):
        return min(abs(t[0] - s[0]) + abs(t[1] - s[1]) for s in sheds)
    ...
    candidates = [
        ...
        for y in range(board_size)
        for x in range(board_size)
        if (x, y) not in set(sheds)
        and shed_dist((x, y)) > PLOT_MIN_SHED_DIST
        and quadrant(x, y) in ownable
    ]
    if not candidates:
        return ()
    candidates.sort(key=lambda t: (shed_dist(t), t))
    return _angular(candidates[:PLOT_SIZE])


def _plot_candidates(board_size: int) -> list[tuple[int, int]]:
    """Every tile a crop plot may use, nearest-to-shed first (the same
    ordering plot_tiles takes its first PLOT_SIZE from)."""
    sheds = shed_tiles(board_size)
    half = board_size // 2
...
def plot2_tiles(board_size: int) -> tuple[tuple[int, int], ...]:
    """The optional second plot: the next PLOT2_SIZE candidate tiles
    after the wheat plot's, so the two never overlap."""
    if PLOT2_SIZE <= 0:
        return ()
    return _angular(_plot_candidates(board_size)[PLOT_SIZE:PLOT_SIZE + PLOT2_SIZE])


def all_plot_tiles(board_size: int) -> frozenset:
    """Every tile reserved for crops -- herd duty must never build here."""
    return frozenset(plot_tiles(board_size)) | frozenset(plot2_tiles(board_size))


def _angular(selected: list) -> tuple:
    """Re-sort by angle around the selection's own centroid, so slicing
    the sequence into per-worker blocks (worker_block) keeps each
    worker's tiles adjacent on the board rather than scattered across
    opposite sides of the ring."""
    if not selected:
        return ()
    cx = sum(t[0] for t in selected) / len(selected)
    cy = sum(t[1] for t in selected) / len(selected)
    return tuple(sorted(selected, key=lambda t: math.atan2(t[1] - cy, t[0] - cx)))


def worker_block(plot: tuple, worker_index: int, worker_count: int) -> tuple:
    """The contiguous slice of the plot one crop worker owns all season."""
    if not plot or worker_count <= 0:
        return ()
    per = max(1, len(plot) // worker_count)
    start = worker_index * per
    end = len(plot) if worker_index == worker_count - 1 else start + per
    return plot[start:end]


def _plot_tile_work(
    obs: GameObservation, pos: tuple[int, int], seeds_left: int | None = None, crop: str = PLOT_CROP
) -> str | None:
    """What the plot tile at `pos` needs now, or None.

    `seeds_left` is this turn's REMAINING seed budget across all crop
    workers (see _decide_crop_action) -- not the raw shed count -- so an
    empty tile only reads as plantable while a seed is still unclaimed.
    """
    tile = obs.tile_at(*pos)
    if tile is None:
        if seeds_left is None:
            seeds_left = obs.seeds.get(crop, 0)
        return "PLANT" if seeds_left > 0 else None
    kind = tile_kind(tile)
    if kind == "WEED":
        return "DIG"
    if kind != "PLANT" or not isinstance(tile, dict) or tile.get("crop") != crop:
        return None
    cd = CROPS[crop]
    if not tile.get("watered_today"):
        # A ONE-TIME crop (WHEAT) takes its yield from the WATER action
        # itself, inside the bonus window -- watered daily. An ONGOING
        # crop produces on its interval regardless and only needs enough
        # water to avoid two consecutive dry days.
        if not cd["ongoing"] or tile.get("consecutive_unwatered", 0) >= 1:
            return "WATER"
    if tile.get("yield_units", 0) > 0:
        if cd["ongoing"] or obs.day - tile["planted_day"] >= cd["max_yield_day"]:
            return "HARVEST"
    return None


def _decide_crop_action(
    obs: GameObservation,
    pos: tuple[int, int],
    block: tuple,
    seed_budget: dict | None = None,
    crop: str = PLOT_CROP,
) -> list[str]:
    """A crop worker tends its own block and nothing else.

    `seed_budget` is shared by every crop worker this turn and decremented
    as each one commits to a PLANT. The engine enforces this atomically
    and harshly: if a turn's PLANT requests for a crop exceed the seeds
    in stock, it drops ALL of them, not just the excess. With three
    workers deciding independently against the same raw seed count,
    that silently turned 25% of v17's PLANT actions into PASS (109 over
    four seasons), and delayed every one of those plantings a turn.
    """
    if seed_budget is None:
        seed_budget = {"left": obs.seeds.get(crop, 0)}
    if not block:
        return ["PASS"]
    if pos in block:
        work = _plot_tile_work(obs, pos, seed_budget["left"], crop)
        if work == "DIG":
            return ["DIG"]
        if work == "PLANT":
            seed_budget["left"] -= 1
        ...
        if work == "HARVEST":
            return ["HARVEST"]
    needy = [t for t in block if _plot_tile_work(obs, t, seed_budget["left"], crop) is not None]
    if needy:
        target = min(needy, key=lambda t: abs(t[0] - pos[0]) + abs(t[1] - pos[1]))
        return _step_toward(pos, target)
    if pos not in block:
        return _step_toward(pos, block[0])
    return ["PASS"]


def _decide_hand_action(
    obs: GameObservation,
    pos: tuple[int, int],
    unit_inv: dict,
    claimed: set,
    placement_duty: bool = False,
) -> list[str]:
    ...
    if kind in ("COOP", "PASTURE") and isinstance(tile, dict) and tile.get("animal"):
        if _should_feed(obs, tile) and unit_inv.get("WHEAT", 0) > 0:
            return ["FEED"]
    ...
    for animal, info in ANIMALS.items():
        ...
        if target:
            claimed.add(target)
            return _step_toward(pos, target)
        if kind == "EMPTY" and pos not in all_plot_tiles(len(obs.my_farm["tiles"])):
    ...
    # On placement duty: skip feed/care errands and head for the shed.
    # Guarded on "not already there" -- _step_toward(pos, pos) is PASS,
    # which would park the unit on top of the waiting animal.
    if placement_duty and pos not in shed_tiles(len(obs.my_farm["tiles"])):
        duty_sheds = shed_tiles(len(obs.my_farm["tiles"]))
        return _step_toward(pos, min(duty_sheds, key=lambda t: abs(t[0] - pos[0]) + abs(t[1] - pos[1])))

    if unit_inv.get("WHEAT", 0) > 0:
        target = _find_nearest_matching(
            obs, pos, claimed,
            lambda t: _should_feed(obs, t),
        )
        if target:
            claimed.add(target)
            return _step_toward(pos, target)
    ...
    if pos in sheds:
        ...
        duty_pending = _any_pending_animal(obs)
        if placement_duty and duty_pending:
            return ["PICKUP", duty_pending, 1]
    ...
    # v14: the shed is NOT an exclusive work site, and must not be
    # claimed. `claimed` exists to stop two units travelling to the same
    # animal tile, where the second one would arrive to find the job
    # already done. The shed is the opposite: it is the farm's only
    # logistics hub, every unit restocks feed there, and the engine has no
    # collision rule -- units share a tile freely.
    #
    # Gating it cost 22.5% of ALL unit-turns: units that had nothing else
    # to do stood still because one other unit was already walking to the
    # shed. That is two thirds of v13's 34% idle rate, and it is why
    # adding hands never improved feeding (the extra hands simply PASSed)
    # and why the herd could not be grown.
...
def _market_orders(obs: GameObservation, target_crop: str | None = None) -> list[list]:
    ...
    # Wheat the herd will actually eat today (see _should_feed) -- what
    # gets BOUGHT. The sell reserve stays one per placed animal.
    feed_need = _count_unfed_animals(obs)
    ...
    # WHEAT purchase for feed goes here -- right after sells, BEFORE the
    # plot seed order and hires. It was previously last in the list, and
    # once other orders competed for the 10-order cap, the wheat buy was
    # measured silently dropped entirely (shed stock near 0 while most of
    # the herd went unfed). Feeding the herd that already exists must
    # always outrank hiring more of it or seeding the crop plot -- even a
    # plot that is itself growing wheat, since it only matures after
    # ~day 5 and the herd needs feeding from day 0.
    #
    # Count wheat already in the pipeline (hand inventories), not just
    # the shed. Hands PICKUP wheat into their own inventory as soon as it
    # lands in the shed, so shed stock alone constantly reads as "low"
    # even when there is already enough in transit for today.
    wheat_available = shed.get("WHEAT", 0) + sum(inv.get("WHEAT", 0) for inv in obs.inventories)
    if feed_need > 0 and wheat_available < feed_need:
        needed = feed_need - wheat_available
    ...
    # WHEAT plot seed purchase: goes BEFORE the hire queue, not after.
    # The hour-0 turn already fills most of the 10-order cap with sells
    # plus up to several hires; a seed order placed after them was
    # silently dropped in an earlier attempt (bug #1 there), so it goes
    # first here and hires give way if the cap binds.
    if PLOT_WORKERS > 0:
        seed_cost = CROPS[PLOT_CROP]["seed"]
        empty = sum(1 for t in plot_tiles(len(obs.my_farm["tiles"])) if obs.tile_at(*t) is None)
        # BUY_SEED takes a QUANTITY: the engine's market loop commits one
        # unit per iteration, so this is one order, not one per seed.
        affordable = int(money // seed_cost)
        want = min(PLOT_SEED_BATCH, empty, affordable) - seeds.get(PLOT_CROP, 0)
        if want > 0:
            orders.append(["BUY_SEED", PLOT_CROP, want])

    # Second-plot seeds only from cash above a floor: at 100 each (for
    # STRAWBERRY) a full batch would otherwise eat the early herd budget
    # that v17's aggressive opening deliberately spends down to ~zero.
    if PLOT2_SIZE > 0 and PLOT2_WORKERS > 0 and money >= PLOT2_CAPITAL_FLOOR:
        seed_cost2 = CROPS[PLOT2_CROP]["seed"]
        empty2 = sum(1 for t in plot2_tiles(len(obs.my_farm["tiles"])) if obs.tile_at(*t) is None)
        spare = int((money - PLOT2_CAPITAL_FLOOR) // seed_cost2)
        want2 = min(PLOT2_SEED_BATCH, empty2, spare) - seeds.get(PLOT2_CROP, 0)
        if want2 > 0:
            orders.append(["BUY_SEED", PLOT2_CROP, want2])

    orders.extend(_hire_orders(obs, money))

    unlocked = obs.unlocked_quadrants()
    if len(unlocked) < MAX_QUADRANTS:
        next_cost = [1000, 2000, 4000][len(unlocked) - 1]
        # See LAND_RESERVE_MULT's docstring for why this is no longer 2x.
        if money >= next_cost * LAND_RESERVE_MULT:
    ...
    if (
        ...
        # An animal that cannot reach its first production before the
        # season ends is pure loss (animals have no resale value).
        and obs.day + ANIMALS[target_animal]["first_yield_day"] + BUY_CUTOFF_MARGIN < SEASON_DAYS
    ):
...
def baseline_strategy(obs: GameObservation) -> dict:
    ...
    n_hands = len(obs.hand_positions())
    plot = plot_tiles(len(obs.my_farm["tiles"]))
    plot2 = plot2_tiles(len(obs.my_farm["tiles"]))
    claimed.update(plot)  # herd duty must never build a pasture on a crop plot
    claimed.update(plot2)

    # The LAST hands work the plot; the farmer and everyone else stays on
    # the herd. Hands are re-hired each morning in a fixed order, so this
    # assignment is stable day to day.
    #
    # v17 tried fixing the resulting index "churn" (as n_hands grows, the
    # last-N indices shift, so a hand can flip between crop and herd duty
    # across days) with a FIXED index range instead. That measured worse,
    # not better -- 8% at index 4, 35% at index 7, both clear losses
    # against this exact "last N" scheme. The churn was not the problem;
    # reverted.
    crop_workers = (
        set(range(max(1, n_hands + 1 - PLOT_WORKERS), n_hands + 1))
        if (PLOT_WORKERS and n_hands >= PLOT_WORKERS + 1)
        else set()
    )
    crop_rank = {idx: r for r, idx in enumerate(sorted(crop_workers))}

    # Second-plot crew: the PLOT2_WORKERS hands just below the wheat crew,
    # only once there are enough hands that the herd still keeps at least
    # one besides the farmer.
    plot2_workers = set()
    if PLOT2_SIZE > 0 and PLOT2_WORKERS and crop_workers:
        top = min(crop_workers) - 1
        if top - PLOT2_WORKERS + 1 >= 2:
            plot2_workers = set(range(top - PLOT2_WORKERS + 1, top + 1))
    plot2_rank = {idx: r for r, idx in enumerate(sorted(plot2_workers))}

    # Placement duty is restricted to non-crop indices: crop workers
    # ignore the flag entirely, so including them in the candidate pool
    # can waste the assignment on a unit that will never act on it.
    duty = _placement_duty_index(obs, exclude=crop_workers | plot2_workers)
    order = list(range(len(positions)))
    if duty is not None and duty < len(order):
        order.remove(duty)
        order.insert(0, duty)  # duty unit decides first so it claims what it needs

    actions: dict[int, list[str]] = {}
    seed_budget = {"left": obs.seeds.get(PLOT_CROP, 0)}
    seed_budget2 = {"left": obs.seeds.get(PLOT2_CROP, 0)}
    for idx in order:
        if idx in crop_workers:
            actions[idx] = _decide_crop_action(
                obs, positions[idx], worker_block(plot, crop_rank[idx], len(crop_workers)), seed_budget
            )
        elif idx in plot2_workers:
            actions[idx] = _decide_crop_action(
                obs,
                positions[idx],
                worker_block(plot2, plot2_rank[idx], len(plot2_workers)),
                seed_budget2,
                PLOT2_CROP,
            )
        else:
            actions[idx] = _decide_hand_action(
                obs, positions[idx], inventories[idx], claimed, placement_duty=idx == duty
            )
    farmer_action = actions[0]
    hands_actions = [actions[i + 1] for i in range(len(obs.hand_positions()))]
    return {
