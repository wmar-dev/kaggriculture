...
def _pending_animals(obs: GameObservation) -> int:
    return obs.shed.get(TARGET_ANIMAL, 0) + sum(inv.get(TARGET_ANIMAL, 0) for inv in obs.inventories)
...
def _decide_hand_action(obs: GameObservation, pos: tuple[int, int], unit_inv: dict, claimed: set) -> list[str]:
    ...
    if pos in sheds:
        ...
        if obs.shed.get(TARGET_ANIMAL, 0) > 0 and _count_placed_animals(obs) + _pending_animals(obs) <= MAX_STRUCTURES:
...
def _market_orders(obs: GameObservation, target_crop: str | None) -> list[list]:
    ...
    if (
        _count_placed_animals(obs) + _pending_animals(obs) < MAX_STRUCTURES
        and money >= animal_cost * REINVEST_RESERVE
    ):
