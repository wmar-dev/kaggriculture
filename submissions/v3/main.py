...
def _count_structures(obs: GameObservation) -> int:
    return sum(
        1
        for row in obs.my_farm["tiles"]
        for tile in row
...
def _decide_hand_action(obs: GameObservation, pos: tuple[int, int], unit_inv: dict, claimed: set) -> list[str]:
    ...
    if pos in sheds:
        ...
        structures_owned = _count_structures(obs)
        pending_animals = obs.shed.get(TARGET_ANIMAL, 0) + sum(
            inv.get(TARGET_ANIMAL, 0) for inv in obs.inventories
        )
        if structures_owned + pending_animals < TARGET_STRUCTURES and obs.shed.get(TARGET_ANIMAL, 0) > 0:
...
def _market_orders(obs: GameObservation, target_crop: str | None) -> list[list]:
    ...
    if structures_owned + pending_animals < TARGET_STRUCTURES and money >= ANIMALS[TARGET_ANIMAL]["cost"]:
