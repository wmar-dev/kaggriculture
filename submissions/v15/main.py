...
def _placement_duty_index(obs: GameObservation) -> int | None:
    """Which single unit (0 = farmer, 1+ = hands) fetches a bought animal.
    ...
    and only while an animal is actually waiting.
    """
...
def baseline_strategy(obs: GameObservation) -> dict:
    ...
    """
    claimed: set = set()
    duty = _placement_duty_index(obs)
    positions = [obs.farmer_position()] + obs.hand_positions()
    inventories = [obs.farmer_inventory()] + [
        obs.hand_inventory(i) for i in range(len(obs.hand_positions()))
    ]
